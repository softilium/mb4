...
// QueryInvestAccounts chains the current query on the "InvestAccounts" edge.
func (uq *UserQuery) QueryInvestAccounts() *InvestAccountQuery {
	query := &InvestAccountQuery{config: uq.config}
...
// WithInvestAccounts tells the query-builder to eager-load the nodes that are connected to
// the "InvestAccounts" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithInvestAccounts(opts ...func(*InvestAccountQuery)) *UserQuery {
	query := &InvestAccountQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withInvestAccounts; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[xid.ID]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.InvestAccounts = []*InvestAccount{}
		}
		query.withFKs = true
		query.Where(predicate.InvestAccount(func(s *sql.Selector) {
			s.Where(sql.InValues(user.InvestAccountsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_invest_accounts
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_invest_accounts" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
